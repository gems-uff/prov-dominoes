...
public class SubtractCommand extends AbstractCommand {
		...
		if (App.getArea().getData().getCombination() == PieceCanvasState.COMBINATION_SUBTRACTION && App.getArea().getData().getIndexFirstOperatorCombination() != -1
			...
			try {
				if (d1.getIdRow().equals(d2.getIdRow()) && d1.getIdCol().equals(d2.getIdCol())) {
					Dominoes resultOperation = provdominoes.control.Controller.subtract(d1, d2);
				...
				App.alertException(e, "Erro desconhecido ao calcular subtração das matrizes das peças!");
				result = false;
				e.printStackTrace();
			}
		}
	...
	public String getName() {
		String base = SUBTRACT_COMMAND + "(" + leftDominoes.getId() + ", " + rightDominoes.getId() + ")";
		return key + " = " + base;
	}
