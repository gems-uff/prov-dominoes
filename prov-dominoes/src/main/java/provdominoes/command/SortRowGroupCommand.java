...
public class SortRowGroupCommand extends AbstractCommand {
	...
	public SortRowGroupCommand() {
		this.index = -1;
	}

	public SortRowGroupCommand(int index) {
		this();
		this.index = index;
	}
	...
	protected boolean doIt() {
		...
		try {
			Dominoes toSortRowCount = App.getArea().getData().getDominoes().get(index);
			Dominoes domino = provdominoes.control.Controller.sortRowCount(toSortRowCount);
	...
	public String getName() {
		return SORT_ROW_GROUP_COMMAND + "(" + this.oldDominoes.getId() + ")";
	}
	private String id;
